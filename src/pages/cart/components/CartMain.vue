...
import { CameraDataMapping } from 'XrFrame/xrFrameSystem'
//
...
const changeCount = async (e: InputNumberBoxEvent) => {
  const res = await putCartCountApi(e.index, { count: e.value })
}
const changeSelected = async (item: CartItem) => {
  item.selected = !item.selected
  const res = await putCartCountApi(item.skuId, { selected: item.selected })
  console.log(res)
}
...
const onSelectAll = () => {
  const sa = !selectedAll.value
  cartList.value?.forEach((item) => {
    item.selected = sa
  })
  putMemberCartSelectedApi({ selected: sa })
}
...
const totalPrice = computed(() => {
  return selectedCardList.value?.reduce((sum, item) => sum + item.count * item.price, 0)
})
</script>
