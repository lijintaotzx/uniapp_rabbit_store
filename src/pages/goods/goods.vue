  ...
  <scroll-view scroll-y class="viewport">
    ...
    <view v-else>
      ...
      <view class="goods">
        <!-- 商品主图 -->
        <view class="preview">
          <swiper circular @change="swiperChange" autoplay>
            <swiper-item v-for="item in goodsDetail?.mainPictures" :key="item">
              <image mode="aspectFill" :src="item" @tap="onTapImg(item)" />
            </swiper-item>
          </swiper>
          <view class="indicator">
            <text class="current">{{ activePictureIndex + 1 }}</text>
...
const swiperChange: UniHelper.SwiperOnChange = (e) => {
  ...
  activePictureIndex.value = e.detail.current
}
const onTapImg = (src: string) => {
  // 大图预览
  uni.previewImage({
    current: src,
