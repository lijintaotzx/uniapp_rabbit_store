...
// 是否是更新页面
const isPut = query.id ? true : false
onLoad(async () => {
  uni.setNavigationBarTitle({ title: isPut ? '修改地址' : '添加地址' })
  if (isPut) {
    const details = await getAddressDetailApi(query.id!)
    form.value = details.result
  }
})
...
const changeDefault: UniHelper.SwitchOnChange = (e) => {
...
const postAddress = async () => {
  ...
  if (res.result.id) {
    ...
    uni.showToast({ icon: 'error', title: '创建失败！' })
  }
}
const putAddress = async () => {
  console.log('put...')
  const res = await putAddressApi({ id: query.id!, ...form.value })
  if (res.result.id) {
    uni.navigateBack()
...
<template>
  ...
  <!-- 提交按钮 -->
  <button class="button" @tap="submitAddress">保存并使用</button>
</template>
